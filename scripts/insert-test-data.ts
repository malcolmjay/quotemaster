// Generate SQL INSERT statements for batch execution
...
function generateCustomersBatch(batchNum: number, batchSize: number, startNum: number): string {
  const values = [];
  const start = batchNum * batchSize + startNum;
  ...
  for (let i = 0; i < batchSize; i++) {
    ...
    values.push(`(gen_random_uuid(),'${customerNumber}','${name.replace(/'/g, "''")}','${type}','${segment}',${contractNumber ? `'${contractNumber}'` : 'NULL'},'${paymentTerms[Math.floor(Math.random() * paymentTerms.length)]}','USD','${tier}','Manager ${Math.floor(Math.random() * 20) + 1}','Rep ${Math.floor(Math.random() * 50) + 1}',now(),now())`);
  }

  return `INSERT INTO customers(id,customer_number,name,type,segment,contract_number,payment_terms,currency,tier,sales_manager,sales_rep,created_at,updated_at)VALUES${values.join(',')};`;
}

function generateProductsBatch(batchNum: number, batchSize: number, startNum: number): string {
  const values = [];
  const start = batchNum * batchSize + startNum;
