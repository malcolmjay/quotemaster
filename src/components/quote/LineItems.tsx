  ...
  return (
    <div className={`rounded p-3 ${isPriceExpired ? 'bg-[#f2dede] border border-[#ebccd1]' : 'bg-[#dff0d8] border border-[#d6e9c6]'}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className={`text-xs font-medium ${isPriceExpired ? 'text-red-600' : 'text-emerald-700'}`}>
          {isPriceExpired ? 'Expired Pricing' : 'Confirmed Pricing'}
        </span>
      </div>
      <div className="grid grid-cols-4 gap-3 text-xs">
        <div>
          <span className="text-[#5f6672]">Supplier Price:</span>
          <div className="font-semibold text-[#1a1f36]">${priceRequest.supplier_pricing?.toFixed(2)}</div>
        </div>
        <div>
          <span className="text-[#5f6672]">MOQ:</span>
          <div className="font-semibold text-[#1a1f36]">{priceRequest.moq || 'N/A'}</div>
        </div>
        <div>
          <span className="text-[#5f6672]">Valid Until:</span>
          <div className={`font-semibold ${isPriceExpired ? 'text-red-600' : 'text-[#1a1f36]'}`}>
            {priceRequest.effective_end_date || 'N/A'}
          </div>
        </div>
        {priceBreaks.length > 0 && (
          <div>
            <span className="text-[#5f6672]">Price Breaks:</span>
...
export const LineItems: React.FC<LineItemsProps> = ({
  ...
  return (
    <div className="bg-white rounded-lg border border-[#dce0e6] shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-[#eef0f3] bg-[#f8f9fb]">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-md bg-[#e8f0fe] flex items-center justify-center">
              <Package className="w-4 h-4 text-[#1a6fb5]" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-[#1a1f36]">Line Items</h3>
              <p className="text-xs text-[#8c939d]">
                {lineItems.length} item{lineItems.length !== 1 ? 's' : ''}
                {expiredCount > 0 && <span className="text-red-600 ml-1 font-medium">({expiredCount} expired)</span>}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <HelpTooltip content="Search for products by SKU, name, or description to add them to your quote. Use Shift++ keyboard shortcut to quickly focus this field.">
              <div className="relative">
                ...
                {showSearchResults && searchResults.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#dce0e6] rounded-lg shadow-lg z-20 max-h-64 overflow-y-auto">
                    {searchResults.slice(0, 10).map((product) => (
                      <button
                        key={product.sku}
                        onClick={() => handleProductSelect(product)}
                        className="w-full px-3 py-2.5 text-left hover:bg-[#f4f5f7] border-b border-[#eef0f3] last:border-b-0 transition-colors"
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="font-medium text-sm text-[#1a1f36]">{product.sku}</div>
                            <div className="text-xs text-[#8c939d] truncate">{product.name}</div>
                          </div>
                          <div className="text-xs font-medium text-[#1a6fb5]">${product.price?.toLocaleString()}</div>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            <HelpTooltip content="Browse the complete product catalog to add items to your quote.">
              <button
                onClick={() => setShowProductModal(true)}
                className="px-3 py-2 text-sm text-[#5f6672] hover:text-[#1a1f36] hover:bg-[#eef0f3] rounded-md transition-colors"
              >
                Browse
              </button>
            </HelpTooltip>

            <div className="h-5 w-px bg-[#dce0e6]" />

            <HelpTooltip content="Upload a CSV file to bulk-add or update line items.">
              <button
                onClick={() => { setCsvUploadMode('add'); setShowCSVUploadModal(true); }}
                className="flex items-center gap-1.5 px-3 py-2 bg-[#1a6fb5] hover:bg-[#155a94] text-white text-sm font-medium rounded-md transition-colors shadow-sm"
              >
            ...
            {lineItems.length > 0 && (
              <HelpTooltip content="Export all line items to a CSV file.">
                <button
                  onClick={exportToCSV}
                  className="flex items-center gap-1.5 px-3 py-2 text-sm text-[#5f6672] hover:text-[#1a1f36] hover:bg-[#eef0f3] rounded-md transition-colors"
                >
        ...
        {selectedItems.length > 0 && (
          <div className="mt-3 flex items-center justify-between py-2.5 px-4 bg-[#e8f0fe] rounded-md border border-[#c4d9f2]">
            <span className="text-sm font-medium text-[#1a6fb5]">
              {selectedItems.length} selected
            </span>
            <div className="flex items-center gap-2">
              <HelpTooltip content="Create price requests for selected items.">
                <button onClick={() => handleBulkAction('price-request')} className="px-3 py-1.5 bg-[#1a6fb5] text-white text-xs font-medium rounded-md hover:bg-[#155a94] transition-colors">
                  Request Pricing
                </button>
              </HelpTooltip>
              <HelpTooltip content="Request current lead time information for selected items.">
                <button onClick={() => handleBulkAction('lead-time-request')} className="px-3 py-1.5 border border-[#1a6fb5] text-[#1a6fb5] text-xs font-medium rounded-md hover:bg-[#e8f0fe] transition-colors">
                  Request Lead Time
                </button>
              </HelpTooltip>
              <HelpTooltip content="Clear the current selection.">
                <button onClick={() => setSelectedItems([])} className="px-3 py-1.5 text-[#5f6672] text-xs hover:bg-white/50 rounded-md transition-colors">
                  Clear
                </button>
              </HelpTooltip>
            </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          <HelpTooltip content="Show or hide advanced filters for SKU, supplier, cost expiration, and status.">
            <button
              onClick={() => setShowFilters(!showFilters)}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs rounded-md transition-colors ${
                hasActiveFilters ? 'bg-[#e8f0fe] text-[#1a6fb5] border border-[#c4d9f2] font-medium' : 'text-[#5f6672] hover:bg-[#eef0f3]'
              }`}
            >
              <Filter className="w-3.5 h-3.5" />
              Filters
              {hasActiveFilters && <span className="ml-1 px-1.5 py-0.5 bg-[#1a6fb5] text-white text-xs rounded-full">{[filterProductNumber, filterSupplier, filterExpiredCost !== 'all', filterStatus !== 'all'].filter(Boolean).length}</span>}
            </button>
          </HelpTooltip>
          ...
          {showFilters && (
            <>
              <input
                ...
                placeholder="SKU"
                className="w-28 px-2.5 py-1.5 text-xs border border-[#dce0e6] rounded-md bg-white"
              />
              <select
                value={filterSupplier}
                onChange={(e) => setFilterSupplier(e.target.value)}
                className="px-2.5 py-1.5 text-xs border border-[#dce0e6] rounded-md bg-white"
              >
              ...
              {hasActiveFilters && (
                <button onClick={clearFilters} className="p-1.5 text-[#8c939d] hover:text-[#1a1f36] hover:bg-[#eef0f3] rounded-md transition-colors">
      ...
      {expiredCount > 0 && (
        <div className="mx-5 mt-4 p-3 bg-red-50 border border-red-200 rounded-md flex items-center gap-3">
          <AlertCircle className="w-4 h-4 text-red-600 flex-shrink-0" />
          <p className="text-xs text-red-700">
            <span className="font-semibold">{expiredCount} item(s) have expired costs.</span> Review pricing before submitting.
          </p>
        </div>
      )}
                ...
                <tr className={`transition-colors ${
                  ...
                  <td className="px-3 py-3 text-center">
                    <input
                      ...
                      className="w-16 px-2 py-1.5 border border-[#dce0e6] dark:border-slate-600 rounded text-center text-sm bg-white dark:bg-slate-700"
                      min="1"
                    />
                  </td>
                  ...
                  <td className="px-3 py-3 text-right">
                    <span className="font-semibold text-sm text-[#1a1f36] dark:text-white">
                  ...
                  <td className="px-3 py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      <div className={`w-1.5 h-1.5 rounded-full ${item.stock > 10 ? 'bg-emerald-600' : item.stock > 0 ? 'bg-[#f0ad4e]' : 'bg-[#d9534f]'}`}></div>
                      <span className="text-sm text-[#1a1f36] dark:text-slate-300">{item.stock}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-3 text-center">
                    <input
                      ...
                      className="w-20 px-2 py-1 border border-[#dce0e6] dark:border-slate-600 rounded text-center text-xs bg-white dark:bg-slate-700"
                    />
                  </td>
                  ...
                  <td className="px-3 py-3 text-center">
                    <select
                      ...
                      className={`px-2 py-1 border rounded text-xs font-medium ${
                        item.status === 'Won' ? 'bg-[#dff0d8] text-emerald-700 border-[#d6e9c6]' :
                        item.status === 'Lost' ? 'bg-[#f2dede] text-red-600 border-[#ebccd1]' :
                        item.status === 'Price Request' ? 'bg-[#e8f0fe] text-[#1a6fb5] border-[#bce8f1]' :
                        'bg-[#f8f9fb] text-[#1a1f36] border-[#dce0e6] dark:bg-slate-700 dark:text-slate-300 dark:border-slate-600'
                      }`}
                    >
                      <option value="Pending">Pending</option>
                  ...
                  <td className="px-3 py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      <button
                        ...
                        title="Cost Analysis"
                      >
                        <Calculator className="w-4 h-4 text-[#5f6672] dark:text-slate-400" />
                      </button>
                      {supplyPeriodMonths > 12 && onShowMultiYearPricing && (
                        ...
                        >
                          <Calendar className="w-4 h-4 text-[#1a6fb5]" />
                        </button>
                      )}
                      <button
                        ...
                        title="View History"
                      >
                        <Eye className="w-4 h-4 text-[#5f6672] dark:text-slate-400" />
                      </button>
                      <button
                        ...
                        title="Messages"
                      >
                        <MessageCircle className="w-4 h-4 text-[#1a6fb5] dark:text-blue-400" />
                      </button>
                      <button
                ...
                <td colSpan={10} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center">
                    <div className="w-16 h-16 rounded-full bg-[#f4f5f7] flex items-center justify-center mb-4">
                      <Package className="h-8 w-8 text-[#cdd1d9]" />
                    </div>
                    <h3 className="text-sm font-semibold text-[#1a1f36] mb-1">No items yet</h3>
                    <p className="text-sm text-[#8c939d] mb-5">Add products to build your quote</p>
                    <div className="flex gap-2">
                      <button onClick={() => setShowProductModal(true)} className="flex items-center gap-1.5 px-4 py-2 bg-[#1a6fb5] text-white text-sm font-medium rounded-md hover:bg-[#155a94] shadow-sm transition-colors">
                        <Plus className="h-4 w-4" /> Add Product
                      </button>
                      <button onClick={() => { setCsvUploadMode('add'); setShowCSVUploadModal(true); }} className="flex items-center gap-1.5 px-4 py-2 border border-[#dce0e6] text-[#5f6672] text-sm rounded-md hover:bg-[#f8f9fb] transition-colors">
            ...
            {lineItems.length > 0 && filteredLineItems.length === 0 && (
              <tr>
                <td colSpan={10} className="px-6 py-12 text-center">
                  <Filter className="h-10 w-10 text-[#cdd1d9] mx-auto mb-3" />
                  <h3 className="text-sm font-semibold text-[#1a1f36] mb-1">No matches</h3>
                  <p className="text-sm text-[#8c939d] mb-3">Adjust your filters</p>
                  <button onClick={clearFilters} className="px-3 py-1.5 text-sm text-[#1a6fb5] hover:bg-[#e8f0fe] rounded-md transition-colors">Clear Filters</button>
                </td>
              </tr>
            )}
          </tbody>
